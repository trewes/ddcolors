...
int DDColors::run() {
    ...
    if(opt.algorithm == Options::HeuristicRefinement) {
        ...
        if(opt.algorithm == Options::ExactCompilation) {
          double ip_flow = compute_flow_solution(dd, IP, upper_bound, opt.formulation, opt.num_cores, opt.MIP_emphasis,opt.exact_ip_file, opt.no_solving);
          if (opt.no_solving) {std::cout << "Exiting w/o solving flow problem." << std::endl; exit(0);}
