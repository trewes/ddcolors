...
unsigned int get_width(const DecisionDiagram &dd){
    return std::max_element(dd.begin(), dd.end(),
            [](const std::vector<Node>& layer1, const std::vector<Node>& layer2){return layer1.size() < layer2.size();})->size();
}

void print_decision_diagram(const DecisionDiagram &dd, bool use_tag) {
    for(const auto& layer : dd){
        std::cout << (use_tag ? "[DD] " : "") << "Layer: " << layer.front().layer << " of size: " << layer.size() << std::endl;
        for (const auto& node : layer){
            std::cout  << (use_tag ? "[DD] " : "") << "Node " << node.index << " state " << node.state_info << " with 1-arc " << node.one_arc << " and 0-arc " << node.zero_arc << std::endl;
        }
    }
}
...
void separate_edge_conflict(DecisionDiagram &dd, const NeighborList &neighbors, const PathLabelConflict &plc,
    ...
    for (int i = 0; i <= (k-1) - j; ++i) {
        ...
        int t = -1;
        auto search_equivalent_node = std::find_if(dd[layer + 1].begin(), dd[layer + 1].end(), [&new_state](const Node& v){return v.state_info == new_state;});
        if(search_equivalent_node == dd[layer + 1].end() ) {
            ...
            if(redirect_arcs == OriginalArcs){
                ...
                auto most_similar_node = std::max_element(dd[layer+2].begin(), dd[layer+2].end(),
                  [&next_state](const Node & u, const Node & v){return intersection_size(next_state, u.state_info) < intersection_size(next_state, v.state_info);});
...
//    std::cout << "found " << conflict_info.size() << " conflicts" << std::endl;
    ...
    if(find_conflicts == LargestFlowConflict){
        ...
        auto max_pos = std::find_if(conflict_flow.begin(), conflict_flow.end(),
                               [&largest_conflict_flow, &largest_up_to](double flow){return flow >= (largest_conflict_flow - largest_up_to);});
...
//    std::cout << solution << std::endl;
    ...
    if (model == LP) {
        ...
        if("Held" and false){
            ...
            double dual_obj = std::accumulate(dual_solution.begin(), dual_solution.begin() + n, double(0.0), [](double sum, double var){return sum+var;});
            ...
            Ktype K = std::min(-(std::numeric_limits<long>::min()+1), std::numeric_limits<long>::max())/(n*num_columns);
            Ktype K2 = std::pow(2, std::floor(std::log2(K)));
            std::cout << -(std::numeric_limits<Ktype>::min()+1) << " and " << std::numeric_limits<Ktype>::max() << " -> " << K << std::endl;
            std::vector<Ktype> int_dual_solution(dual_solution.size());
            std::transform(dual_solution.begin(), dual_solution.end(), int_dual_solution.begin(), [&K](double var){return std::floor(K*var);});
            std::cout << "max of dual solution is " << *std::max_element(dual_solution.begin(), dual_solution.end())  << " with sum " << std::setprecision(200) << dual_solution[0]*K << std::endl;
            std::cout << int_dual_solution << std::endl;
            Ktype int_dual_obj = std::accumulate(int_dual_solution.begin(), int_dual_solution.begin() + n, Ktype (0), [](Ktype sum, Ktype var){return sum+var;});
            long double scaled_int_dual_obj = int_dual_obj/(long double)K;
            std::cout << int_dual_obj << " and reduced again: " <<  std::setprecision(20) << scaled_int_dual_obj << std::endl;
            if(int_dual_obj < K*flow_val) std::cout << "test here" << std::endl; else std::cout << "other test here" << std::endl;
            std::cout << "test alternative: " << std::setprecision(20) << int_dual_obj << " vs " << K*flow_val << std::endl;
            if(flow_val < scaled_int_dual_obj) std::cout << std::setprecision(200) << flow_val << "!=" << scaled_int_dual_obj << " Not matching obj and dual obj values: " << std::abs(flow_val - scaled_int_dual_obj) << std::endl;
            if(flow_val < scaled_int_dual_obj ) throw std::runtime_error("Not matching obj and scaled int dual obj values: " + std::to_string(std::abs(flow_val - scaled_int_dual_obj)));
            ...
            for(int layer = 0; layer < n; layer++) {
                for(int index = 0; index < int(dd[layer].size()); index++) {
                    ...
                    if(u.one_arc != -1){
                        long double res_val;
                        if(layer == 0){
                            res_val = (long double)1*dual_solution[layer] - 1                                                     + 1 * dual_solution[n + shifted_node_indices[layer+1] + u.one_arc];
                        } else if(layer == n-1){
                            res_val = (long double)1*dual_solution[layer] - 1* dual_solution[n + shifted_node_indices[layer] + index] + 0                                                           ;
                        } else{
                            res_val = (long double)1*dual_solution[layer] - 1* dual_solution[n + shifted_node_indices[layer] + index] + 1 * dual_solution[n + shifted_node_indices[layer+1] + u.one_arc];
                        }
                    ...
                    if(u.zero_arc != -1){
                        long double res_val;
                        if(layer == 0){
                            res_val = (long double)0*dual_solution[layer] - 1                                                     + 1 * dual_solution[n + shifted_node_indices[layer+1] + u.zero_arc];
                        } else if(layer == n-1){
                            res_val = (long double)0*dual_solution[layer] - 1* dual_solution[n + shifted_node_indices[layer] + index] + 0                                                            ;
                        } else{
                            res_val = (long double)0*dual_solution[layer] - 1* dual_solution[n + shifted_node_indices[layer] + index] + 1 * dual_solution[n + shifted_node_indices[layer+1] + u.zero_arc];
                        }
                        residual.push_back(res_val);
                    }
                }
...
//    label.reserve(n);
    ...
    for(unsigned int layer = n-1; layer > 0; layer--){
        for(unsigned int i = 0, num_paths = paths.size(), new_num_paths = num_paths; i < num_paths; i++){
            ...
            for(int more_prevs = 1; more_prevs < prev[layer][last_node].size(); more_prevs++){ //for each additional prev copy current path
...
//        std::cout << "A path is: " << paths[path]  << " with length " << std::accumulate(labels[path].begin(), labels[path].end(), int(0))<< std::endl;
    ...
    for(int path = 0; path < paths.size(); path++) {
        for (int j = 1; j < int(paths[path].size()); j++) { //this way we find the first smallest conflict, important assumption for Algorithm 1
